...
const TourPackageList = () => {
  ...
  const fetchPackages = async () => {
    try {
      setLoading(true);
      const response = await tourTrackingService.getPackages();
    ...
    <Stack spacing={3}>
      ...
      <Grid container spacing={0}>
        {filteredPackages.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((pkg) => (
          <Grid
            onClick={() => navigate(`${TOUR_GUIDE_URLs.PACKAGE.DETAILS}?id=${pkg.tourId}`)}
            sx={{ paddingRight: 2, paddingBottom: 2, cursor: 'pointer' }}
            item
            xs={12}
            sm={6}
            md={4}
