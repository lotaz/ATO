...
const WithdrawalDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchWithdrawal = async () => {
      try {
        const response = await get(`/withdrawl/${id}`);
      ...
      <Paper sx={{ p: 3, boxShadow: 3 }}>
        <Stack spacing={3}>
          ...
          <Stack spacing={1}>
            ...
            <MultipleFileUploader disabled={true} values={transactionImgs} onChange={setTransactionImgs} maxFiles={5} />
          </Stack>

          <Box display="flex" justifyContent="flex-end" gap={2}>
            <Button variant="outlined" onClick={() => navigate(WITHDRAWAL_URLs.INDEX)} sx={{ minWidth: 120 }}>
              Quay lại
            </Button>
          </Box>
        </Stack>
